...
const BCU_CHEF_ABI = [
  ...
  },
  {
    inputs: [
      ...
      {internalType: 'contract BiscuitToken', name: '_rewardToken', type: 'address'},
      ...
      {internalType: 'uint256', name: '_feeForDep', type: 'uint256'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'contract BiscuitToken', name: 'rewardToken', type: 'address'},
      ...
      {internalType: 'uint256', name: 'feeForDep', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'contract BiscuitToken', name: '_rewardToken', type: 'address'}],
    name: 'totalLock',
    outputs: [{internalType: 'uint256', name: '', type: 'uint256'}],
    stateMutability: 'view',
    type: 'function',
...
async function loadBscBiscuitContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBscBiscuitPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printBiscuitChefPool(
        App,
        chefAbi,
        chefAddress,
        prices,
