...
const DODO_CHEF_ABI = [
  ...
  },
  {
    inputs: [
      ...
      {internalType: 'address', name: '_ecoAddr', type: 'address'},
    ],
    name: 'initialize',
    outputs: [],
    stateMutability: 'nonpayable',
  ...
  {
    inputs: [
      {internalType: 'uint256', name: '_lep', type: 'uint256'},
  ...
  {inputs: [], name: 'renounceGovernorship', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {
    inputs: [],
    name: 'rewardDelta',
...
const HELMET_CHEF_ABI0 = [
  ...
  },
  {
    ...
    inputs: [
      {indexed: true, internalType: 'address', name: 'user', type: 'address'},
      {indexed: false, internalType: 'uint256', name: 'reward2', type: 'uint256'},
    ],
    name: 'RewardPaid2',
    type: 'event',
  },
  ...
  {inputs: [], name: 'getDoubleReward', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {inputs: [], name: 'getReward', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {inputs: [], name: 'getReward2', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {
    inputs: [],
    name: 'getRewardForDuration',
    outputs: [{internalType: 'uint256', name: '', type: 'uint256'}],
  ...
  },
  {
    inputs: [
      ...
      {internalType: 'address', name: '_rewardsToken', type: 'address'},
      {internalType: 'address', name: '_stakingToken', type: 'address'},
      {internalType: 'address', name: '_stakingPool2', type: 'address'},
      {internalType: 'address', name: '_rewardsToken2', type: 'address'},
      {internalType: 'uint256', name: '_pid2', type: 'uint256'},
    ],
    name: 'initialize',
    outputs: [],
    stateMutability: 'nonpayable',
...
async function loadHelmetSynthetixPoolsInfo2(App, tokens, prices, stakingAbi, stakingAddress) {
  ...
  let newPriceAddresses = stakeToken.tokens.filter(x => !getParameterCaseInsensitive(prices, x))
  let newPrices = await lookUpTokenPrices(newPriceAddresses)
  for (const key in newPrices) {
    if (newPrices[key]?.usd) prices[key] = newPrices[key]
  }
...
async function printHelmetSynthetixPool(App, info, chain = 'bsc') {
  ...
  _print(
    `${info.rewardTokenTicker0} Per Week: ${info.weeklyHelmetRewards.toFixed(2)} ($${formatMoney(info.usdPerWeek0)})`
  )
  ...
  const claim0 = `${info.earned0.toFixed(6)} ${info.rewardTokenTicker0} ($${formatMoney(
    info.earned0 * info.rewardTokenPrice0
  )}`
  const claim1 = `${info.earned1.toFixed(6)} ${info.rewardTokenTicker1} ($${formatMoney(
    info.earned1 * info.rewardTokenPrice1
  )}`
