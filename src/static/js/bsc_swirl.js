...
const SWIRL_CHEF_ABI0 = [
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accSwirlPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_swirlPerBlock', type: 'uint256'}],
    name: 'setSwirlPerBlock',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{internalType: 'contract IERC20', name: '_swirl', type: 'address'}],
    name: 'setSwirlToken',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
const SWIRL_CHEF_ABI1 = [
  ...
  {
    inputs: [
      {internalType: 'contract IERC20', name: '_swirl', type: 'address'},
...
async function main() {
  ...
  const rewardsPerWeek0 = (((await SWIRL_CHEF0.swirlPerBlock()) / 1e18) * 604800) / 3
  const rewardsPerWeek1 = (((await SWIRL_CHEF1.swirlPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  let p0 = await loadSwirlChefContract0(
    App,
    tokens,
    prices,
    SWIRL_CHEF0,
  ...
  let p1 = await loadSwirlChefContract1(
    App,
    tokens,
    prices,
    SWIRL_CHEF1,
...
async function loadSwirlChefContract0(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getSwirlPoolInfo0(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
...
async function loadSwirlChefContract1(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getSwirlPoolInfo1(App, chefContract, chefAddress, pendingRewardsFunction)
    )
  )
