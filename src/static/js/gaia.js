...
function printGaiaContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
...
async function loadGaiaContract(App, chef, chefAddress, chefAbi, rewardTokenTicker) {
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getGaiaPoolInfo(App, chefContract, chefAddress, x, rewardToken.decimals)
    )
  )
...
const GAIA_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract IERC20', name: '_gaia', type: 'address'},
      {internalType: 'uint256', name: '_gaiaPerBlock', type: 'uint256'},
    ],
  ...
  },
  {
    inputs: [
      {internalType: 'uint256', name: '_poolID', type: 'uint256'},
      {internalType: 'uint256', name: '_newEndBlock', type: 'uint256'},
    ],
    name: 'setEndBlock',
    outputs: [],
    stateMutability: 'nonpayable',
  ...
  {
    inputs: [{internalType: 'uint256', name: '_newGaiaPerBlock', type: 'uint256'}],
    name: 'setGaiaPerBlock',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
