...
const PEFI_CHEF_ABI = [
  {
    ...
    inputs: [
      {type: 'address', name: '_pefi', internalType: 'contract PenguinToken'},
      {type: 'address', name: '_devaddr', internalType: 'address'},
      {type: 'address', name: '_feeAddress', internalType: 'address'},
      {type: 'uint256', name: '_pefiPerBlock', internalType: 'uint256'},
  ...
  },
  {
    ...
    inputs: [
      {type: 'address', name: 'user', internalType: 'address', indexed: true},
      {type: 'uint256', name: 'pefiPerBlock', internalType: 'uint256', indexed: false},
    ],
    anonymous: false,
  },
  {
  ...
  },
  {
    type: 'function',
    stateMutability: 'view',
    outputs: [{type: 'address', name: '', internalType: 'contract PenguinToken'}],
    name: 'pefi',
    inputs: [],
  },
  {
  ...
  },
  {
    ...
    outputs: [],
    name: 'updateEmissionRate',
    inputs: [{type: 'uint256', name: '_pefiPerBlock', internalType: 'uint256'}],
  },
  {
    type: 'function',
    stateMutability: 'nonpayable',
...
const xPEFI_ABI = [
  {
    type: 'constructor',
    stateMutability: 'nonpayable',
    inputs: [{type: 'address', name: '_pefi', internalType: 'contract PenguinToken'}],
  },
  {
    type: 'event',
    name: 'Approval',
  ...
  {type: 'function', stateMutability: 'nonpayable', outputs: [], name: 'renounceOwnership', inputs: []},
  {
    ...
    inputs: [{type: 'uint256', name: '_earlyWithdrawalFee', internalType: 'uint256'}],
  },
  {
    type: 'function',
    stateMutability: 'view',
...
const PEFI_ABI = [
  {
    type: 'constructor',
    stateMutability: 'nonpayable',
    inputs: [{type: 'address', name: '_minterAddress', internalType: 'address'}],
  },
  {
    type: 'event',
    name: 'Approval',
...
async function main() {
  ...
  const rewardsPerWeek = (((await PEFI_CHEF.pefiPerBlock()) / 1e18) * 604800) / blocksPerSeconds
...
async function loadXPefi(App, prices) {
  ...
  _print(
    `\n${rewardTicker} Price: $${toFixed(xPefiPrice, 2)} Total Supply: ${toFixed(
      totalXpefiTokens,
      4
    )} TVL: $${formatMoney(tvl)}`
  )
  ...
  _print(
    `Your balance is ${totalOwnedXPefi.toFixed(2)} ${rewardTicker} (${totalOwnedPefi.toFixed(
      2
    )} ${stakeTicker}), $${formatMoney(usersXPefiUsd)}, ${usersPercentage.toFixed(2)}% of the pool`
  )
...
const contract_unstake = async function(abi, contractAddress, stakedAmount, App) {
  ...
  showLoading()
  xPEFI_WRITE_CONTRACT
    .leave(stakedAmount, {gasLimit: 500000})
    .then(function(t) {
      return App.provider.waitForTransaction(t.hash)
    })
    .catch(function() {
