...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.ZAI.DAO.address, Contracts.ZAI.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.ZAI.ZAI.address, ERC20_ABI, App.provider)
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.ZAI.Uniswap_DAI_ZAI.address,
    Contracts.ZAI.LPIncentivizationPool.address,
    tokens,
    prices,
    240,
    true
  ...
  const LP = new ethers.Contract(
    Contracts.ZAI.LPIncentivizationPool.address,
    Contracts.ZAI.LPIncentivizationPool.abi,
    App.provider
  )
  await loadEmptySetLP(App, LP, Contracts.ZAI.Uniswap_DAI_ZAI.address, 'DAI-ZAI LP', 144, epoch, 'ZAI', uniPrices)
