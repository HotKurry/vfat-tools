...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.DSD.DAO.address, Contracts.DSD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.DSD.DSD.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.DSD.Uniswap_DSD_USDC.address,
    Contracts.DSD.LPIncentivizationPool.address,
    tokens,
    prices,
    36
  )
  ...
  const LP = new ethers.Contract(
    Contracts.DSD.LPIncentivizationPool.address,
    Contracts.DSD.LPIncentivizationPool.abi,
    App.provider
  )
  await loadEmptySetLP(App, LP, Contracts.DSD.Uniswap_DSD_USDC.address, 'DSD-USDC LP', 12, epoch, 'DSD', uniPrices)
