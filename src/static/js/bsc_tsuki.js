...
const TSUKI_STAKING_ABI = [
  ...
  },
  {
    ...
    inputs: [
      ...
      {name: '_bnbc', type: 'address'},
      {name: '_fairDistribution', type: 'bool'},
    ],
    name: 'initialize',
    outputs: [],
    payable: false,
...
async function loadTsukiSynthetixPools(App, tokens, prices, pools) {
  ...
  const infos = await Promise.all(
    pools.map(p => loadTsukiPoolInfo(App, tokens, prices, p.abi, p.address, p.tokenRewardAddress, p.stakeTokenFunction))
  )
...
async function loadTsukiPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
