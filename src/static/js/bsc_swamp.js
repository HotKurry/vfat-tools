...
async function main() {
  ...
  const rewardsPerWeek = (((await SWAMP_CHEF.NATIVEPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscSwampContract(
    App,
    tokens,
    prices,
    SWAMP_CHEF,
...
async function loadBscSwampContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async x => await getBscSwampPoolInfo(App, chefContract, x, pendingRewardsFunction))
  )
...
function printSwampContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
