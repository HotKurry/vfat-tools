...
const Pools = ['0x9494b87a5bc8D7Be31f88821E5081Dc5aB029EB2', '0x055c2E794c6e1308B7a1B4c7b80aAf5Ed757c2F6'].map(a => {
  return {
    address: a,
...
async function loadMultipleLudusSynthetixPools(App, tokens, prices, pools) {
  ...
  for (const pool of pools) {
    let info = await loadLudusPoolInfo(
      App,
      tokens,
      prices,
      pool.abi,
...
async function loadLudusPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
  ...
  const calls = [
    STAKING_MULTI.FARMING_END_TIMESTAMP(),
    STAKING_MULTI.rewardRate(),
    STAKING_MULTI.balanceOf(App.YOUR_ADDRESS),
    STAKING_MULTI.pendingReward(App.YOUR_ADDRESS),
  ]
