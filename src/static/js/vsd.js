...
const loadVSDDAO = async (App, DAO, DOLLAR) => {
  ...
  const totalBonded = (
    await Promise.all(
      Contracts.VSD.Pools.map(async pool => {
  ...
  _print(
    `You have ${staged.toFixed(decimals)} staged ${dollar}, $${formatMoney(staged * vsdPrice)}, ${
      totalStaged > 0 ? ((staged / totalStaged) * 100).toFixed(4) : '0'
    }% of the pool`
  )
  _print(
    `You have ${bonded.toFixed(decimals)} bonded ${dollar}, $${formatMoney(bonded * vsdPrice)}, ${(
      (bonded / totalBonded) *
      100
...
async function loadValueSetLP(App, DAO, LP, fluidEpochs, epoch, rewardTicker, uniPrices, dollarPrice, unstakedDollar) {
  ...
  _print(
    `You have ${staged.toFixed(8)} staged ${LP.ticker}, $${formatMoney(staged * lpPrice)}, ${(
      (staged / (totalStaged || 1)) *
      100
    ).toFixed(4)}% of the pool`
  )
  if (staged > 0) uniPrices.print_contained_price(staged)
  _print(
    `You have ${bonded.toFixed(8)} bonded ${LP.ticker}, $${formatMoney(bonded * lpPrice)}, ${(
      (bonded / (totalBonded || 1)) *
      100
    ).toFixed(4)}% of the pool`
  )
  ...
  const TokenToVSDRatio =
    token0.toLowerCase() === LP.baseCoin.toLowerCase()
...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.VSD.DAO.address, Contracts.VSD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.VSD.Dollar.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
...
const vsd_provide = async (DAO, App, LP, vsdToDep) => {
  ...
  if (vsdToDep > 0) {
    showLoading()
    DAO.connect(signer)
      .provide(LP.address, Contracts.VSD.Dollar.address, LP.baseCoin, vsdToDep, {gasLimit: 500000})
      .then(function(t) {
...
const vsd_claim = async function(DAO, App, LP) {
  ...
  if (claimable > 0) {
    showLoading()
    DAO.connect(signer)
      .claim(LP.address, claimable, {gasLimit: 500000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
async function compute30DayYieldRate(App, DAO, twap, epoch) {
  ...
  let filtered = increases
    .filter(prop => prop.args.epoch >= epoch - 89)
    .filter(prop => prop.args.newReward != '0')
    .map(event => {
      return {newReward: event.args.newReward, blockNumber: event.blockNumber}
    })
