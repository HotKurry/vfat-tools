...
const CYC_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract IMintableToken', name: '_cycToken', type: 'address'},
      {internalType: 'contract IERC20', name: '_lpToken', type: 'address'},
      {internalType: 'address', name: '_router', type: 'address'},
      {internalType: 'contract IERC20', name: '_wrappedCoin', type: 'address'},
    ],
    payable: false,
  ...
  },
  {
    ...
    inputs: [
      {indexed: false, internalType: 'uint256', name: 'amount', type: 'uint256'},
      {indexed: false, internalType: 'bool', name: 'isBlockReward', type: 'bool'},
    ],
    name: 'RewardAdded',
    type: 'event',
  },
  ...
  {
    constant: false,
    inputs: [{internalType: 'uint256', name: '_entranceFeeRate', type: 'uint256'}],
    name: 'setEntranceFeeRate',
    outputs: [],
    payable: false,
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const rewardsPerWeek = (((await CYC_CHEF.rewardPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadCycloneChefContract(
    App,
    tokens,
    prices,
    CYC_CHEF,
...
async function loadCycloneChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getCyclonePoolInfo(App, chefContract, chefAddress, pendingRewardsFunction)
    )
  )
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printCycloneChefPool(
        App,
        chefAbi,
        chefAddress,
        prices,
...
function printCycloneContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
