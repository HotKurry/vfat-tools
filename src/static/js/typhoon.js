...
const TYPHOON_ABI = [
  {
    inputs: [
      {internalType: 'contract Tornado', name: '_tornado', type: 'address'},
      {internalType: 'contract IERC20', name: '_typhoon', type: 'address'},
    ],
    payable: false,
  ...
  {
    anonymous: false,
    inputs: [{indexed: true, internalType: 'bytes32', name: 'nullifierHash', type: 'bytes32'}],
    name: 'Staked',
    type: 'event',
  },
  {
  ...
  },
  {
    ...
    inputs: [],
    name: 'tornado',
    outputs: [{internalType: 'contract Tornado', name: '', type: 'address'}],
    payable: false,
    stateMutability: 'view',
    type: 'function',
  },
  {
    constant: true,
    inputs: [],
    name: 'tornadoInputToken',
    outputs: [{internalType: 'contract ERC20Detailed', name: '', type: 'address'}],
    payable: false,
    stateMutability: 'view',
    type: 'function',
  },
...
async function loadMultipleSynthetixPools_(App, tokens, prices, pools) {
  ...
  const infos = await Promise.all(
    pools.map(p => loadSynthetixPoolInfo_(App, tokens, prices, p.abi, p.address, p.stakeTokenFunction, p.name))
  )
...
const LP_ABI = [
  {
    inputs: [
      {internalType: 'contract IERC20', name: '_typhoon', type: 'address'},
      {internalType: 'contract IERC20', name: '_rewardLP', type: 'address'},
    ],
    payable: false,
    stateMutability: 'nonpayable',
    type: 'constructor',
...
const LPPools = [
  {
    name: 'PHOON-USDT',
    address: '0x7df50b4fcae42581e40e2d91edd7d4fa1b37bae1',
    stakeTokenFunction: 'rewardLP',
    rewardTokenFunction: 'typhoon',
    abi: LP_ABI,
  },
  {
    name: 'PHOON-MIC',
    address: '0x7D66Dcd9064272051656D961a25170DD50cDe526',
    stakeTokenFunction: 'rewardLP',
    rewardTokenFunction: 'typhoon',
    abi: LP_ABI,
  },
...
async function main() {
  ...
  var newPriceAddresses = slp.tokens.filter(
    x => x.toLowerCase() !== '0x79256db1bdb6259315a1a3d7dd237f69cadfd8fc' && !getParameterCaseInsensitive(prices, x)
  )
  var newPrices = await lookUpTokenPrices(newPriceAddresses)
  for (const key in newPrices) {
    if (newPrices[key]) prices[key] = newPrices[key]
