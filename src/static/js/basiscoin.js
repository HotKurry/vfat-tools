...
async function loadPool(App, tokens, prices, stakingAbi, stakingAddress, rewardTokenFunction, stakeTokenFunction) {
  ...
  const allowance = stakeTokenFunction == 'lpt' ? userUnstaked : Math.min(20000 - userStaked, userUnstaked)
  const maxAllowance = stakeTokenFunction == 'lpt' ? null : ethers.BigNumber.from(20000).pow(stakeToken.decimals)
