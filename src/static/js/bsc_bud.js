...
const BUD_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract BudToken', name: 'bud', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'address', name: '_feeAddress', type: 'address'},
      {internalType: 'uint256', name: '_budPerBlock', type: 'uint256'},
  ...
  },
  {
    inputs: [],
    name: 'bud',
    outputs: [{internalType: 'contract BudToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accBudPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_budPerBlock', type: 'uint256'}],
...
async function main() {
  ...
  const BUD_CHEF_ADDR = '0x333643dcB46a35a564E63a377F0f65A2108A2A41'
  const rewardTokenTicker = 'BUD'
  const BUD_CHEF = new ethers.Contract(BUD_CHEF_ADDR, BUD_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await BUD_CHEF.budPerBlock()) / 1e18) * 604800) / 3
