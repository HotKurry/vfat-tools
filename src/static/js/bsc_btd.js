...
async function main() {
  ...
  const treasuryContract = new ethers.Contract(
    Contracts.BATDOLLAR['TREASURY'].address,
    Contracts.BATDOLLAR['TREASURY'].abi,
  ...
  var hours_until_next_epoch = Math.floor(seconds_until_next_epoch / 3600)
  seconds_until_next_epoch = seconds_until_next_epoch - hours_until_next_epoch * 3600
  var minutes_until_next_epoch = Math.floor(seconds_until_next_epoch / 60)
  seconds_until_next_epoch = seconds_until_next_epoch - minutes_until_next_epoch * 60
  ...
  _print('BONDS')
  _print(
    `Bonds left this epoch: ${parseFloat(bonds_left / 1e18).toFixed(0)} (without taking max debt ratio into account)`
  )
  _print(
    `Current debt ratio: ${parseFloat((BTB_totalSupply * 100) / BTD_totalSupply).toFixed(
      2
    )}% (Max debt ratio: ${parseFloat(maxDebtRatioPercent / 100).toFixed(2)}%)`
  )
  ...
  for (const pool_nr in pools) {
    ...
    const STAKING_POOL = new ethers.Contract(
      Contracts.BATDOLLAR[key].address,
      Contracts.BATDOLLAR[key].abi,
      App.provider.getSigner()
    )
    ...
    const stakedToken = new ethers.Contract(
      stakedTokenAddress,
      Contracts.BATDOLLAR[stakedTokenKey].abi,
      App.provider.getSigner()
    )
    ...
    _print(
      `Price: <b>1 LP token = $${parseFloat(tokenPrice).toFixed(4)}</b> = ${parseFloat(tokenPerLP).toFixed(7)} ${
        Contracts.BATDOLLAR[key].token0
      } * $${formatMoney(token_price)} + ${parseFloat(busdPerLP).toFixed(4)} BUSD * $${formatMoney(busd_price)}`
    )
    /*
...
*/
    ...
    table =
      `<table style="width:800px"><thead><th style="width:30px;text-align:left"></th><th style="width:20px;text-align:left">Epoch (${period} hours)</th><th style="width:20px;text-align:left">Day</th><th style="width:20px;text-align:left">Week</th><th style="width:20px;text-align:left">Month</th><th style="width:20px;text-align:left">Year</th></thead>` +
      `<tbody>` +
      `<tr><td>APR</td>` +
      ...
      `<td>${parseFloat(userStaked * epochRewardsPerLP).toFixed(2)} BTS ($${formatMoney(
        BTS_price * userStaked * epochRewardsPerLP
      )})</td>` +
      `<td>${parseFloat(userStaked * epochRewardsPerLP * (24 / period)).toFixed(2)} BTS ($${formatMoney(
        BTS_price * userStaked * epochRewardsPerLP * (24 / period)
      )})</td>` +
      `<td>${parseFloat(userStaked * epochRewardsPerLP * (24 / period) * 7).toFixed(2)} BTS ($${formatMoney(
        BTS_price * userStaked * epochRewardsPerLP * (24 / period) * 7
      )})</td>` +
      `<td>${parseFloat((userStaked * epochRewardsPerLP * (24 / period) * 365) / 12).toFixed(2)} BTS ($${formatMoney(
        (BTS_price * userStaked * epochRewardsPerLP * (24 / period) * 365) / 12
      )})</td>` +
      `<td>${parseFloat(userStaked * epochRewardsPerLP * (24 / period) * 365).toFixed(2)} BTS ($${formatMoney(
        BTS_price * userStaked * epochRewardsPerLP * (24 / period) * 365
      )})</td>` +
      `</tr>` +
      `</tbody></table>`
    _print(table)
    ...
    _print(
      `You have staked <b>${parseFloat(userStaked).toFixed(4)} ${stakedTokenKey} ($${formatMoney(
        userStakedUsd
      )})</b> = ${parseFloat(tokenPerLP * userStaked).toFixed(6)} ${Contracts.BATDOLLAR[key].token0} ($${parseFloat(
        tokenPerLP * userStaked * token_price
      ).toFixed(5)}) + ${parseFloat(userStaked * busdPerLP).toFixed(4)} BUSD ($${parseFloat(
        userStaked * busdPerLP * busd_price
      ).toFixed(5)})`
    )
    _print(
      `You can claim <b>${pendingHarvest.toFixed(4)} ${rewardToken} ($${formatMoney(pendingHarvest * BTS_price)})</b>`
    )

    _print('')
    _print_link(
      `Approve ${parseFloat(stakedTokenBalance / 10 ** decimal).toFixed(4)}  ${
        Contracts.BATDOLLAR[stakingTokenTicker].symbol
      }`,
      approveFunc
    )
    _print_link(
      `Stake ${parseFloat(stakedTokenBalance / 10 ** decimal).toFixed(4)}  ${
        Contracts.BATDOLLAR[stakingTokenTicker].symbol
      }`,
      stakeFunc
    )
    _print_link(
      `Unstake ${parseFloat(userStaked).toFixed(4)}  ${Contracts.BATDOLLAR[stakingTokenTicker].symbol}`,
      unstakeFunc
    )
  ...
  const boardRoomContract = new ethers.Contract(
    Contracts.BATDOLLAR[boardRoomKey].address,
    Contracts.BATDOLLAR[boardRoomKey].abi,
    App.provider.getSigner()
  )
  ...
  table =
    ...
    `<td>${parseFloat(inBoardRoom * epochRewardsPerShare).toFixed(2)} BTD ($${formatMoney(
      BTD_price * inBoardRoom * epochRewardsPerShare
    )})</td>` +
    `<td>${parseFloat(inBoardRoom * epochRewardsPerShare * (24 / period)).toFixed(2)} BTD ($${formatMoney(
      BTD_price * inBoardRoom * epochRewardsPerShare * (24 / period)
    )})</td>` +
    `<td>${parseFloat(inBoardRoom * epochRewardsPerShare * (24 / period) * 7).toFixed(2)} BTD ($${formatMoney(
      BTD_price * inBoardRoom * epochRewardsPerShare * (24 / period) * 7
    )})</td>` +
    `<td>${parseFloat((inBoardRoom * epochRewardsPerShare * (24 / period) * 365) / 12).toFixed(2)} BTD ($${formatMoney(
      (BTD_price * inBoardRoom * epochRewardsPerShare * (24 / period) * 365) / 12
    )})</td>` +
    `<td>${parseFloat(inBoardRoom * epochRewardsPerShare * (24 / period) * 365).toFixed(2)} BTD ($${formatMoney(
      BTD_price * inBoardRoom * epochRewardsPerShare * (24 / period) * 365
    )})</td>` +
    `</tr>` +
    `</tbody></table>`
  _print(table)
  ...
  _print(
    `Your claimable rewards: <b>${parseFloat(boardRoomEarned).toFixed(2)} BTD ($${formatMoney(
      boardRoomEarned * BTD_price
    )})</b>`
  )
