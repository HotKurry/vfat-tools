...
async function main() {
  ...
  for (const a of SteelPoolAddresses) {
    ...
    let p = await loadSteelChefContract(
      App,
      tokens,
      prices,
      STEEL_CHEF,
...
async function loadSteelChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBscSteelPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
...
function printIronChefContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
