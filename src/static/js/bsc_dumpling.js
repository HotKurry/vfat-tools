...
const DUMP_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract DumplingToken', name: '_dump', type: 'address'},
  ...
  },
  {
    inputs: [],
    name: 'dump',
    outputs: [{internalType: 'contract DumplingToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accDumpPerShare', type: 'uint256'},
...
async function main() {
  ...
  const DUMP_CHEF_ADDR = '0xa439008F5132e3EB84DFd3f5fE8A0A1aB3D095D1'
  const rewardTokenTicker = 'DUMP'
  const DUMP_CHEF = new ethers.Contract(DUMP_CHEF_ADDR, DUMP_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await DUMP_CHEF.dumpPerBlock()) / 1e18) * 604800) / 3
