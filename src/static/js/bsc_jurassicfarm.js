...
const DINO_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract DinoToken', name: '_dino', type: 'address'},
  ...
  },
  {
    inputs: [],
    name: 'dino',
    outputs: [{internalType: 'contract DinoToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  {
    inputs: [{internalType: 'bool', name: '_autoBuyback', type: 'bool'}],
    name: 'setAutobuyback',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const DINO_CHEF_ADDR = '0x318c3AE7423Cc82F9EC524353Ec2526f72D70386'
  const rewardTokenTicker = 'DINO'
  const DINO_CHEF = new ethers.Contract(DINO_CHEF_ADDR, DINO_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await DINO_CHEF.dinoPerBlock()) / 1e18) * 604800) / 3
