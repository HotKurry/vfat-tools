...
const FLEX_STAKING_ABI = [
  {
    inputs: [
      ...
      {internalType: 'uint256', name: '_maximumContribution', type: 'uint256'},
    ],
  ...
  },
  {
    inputs: [],
    name: 'whitelist',
    outputs: [{internalType: 'contract IWhitelist', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
...
const DASHBORAD_ABI = [
  ...
  },
  {
    ...
    name: 'valueOfAsset',
    outputs: [
      {internalType: 'uint256', name: 'valueInBNB', type: 'uint256'},
      {internalType: 'uint256', name: 'valueInUSD', type: 'uint256'},
    ],
    stateMutability: 'view',
    type: 'function',
  },
...
async function main() {
  ...
  for (let i = 0; i < Pools.length; i++) {
    let p = await loadBscSynthetixPool(
      App,
      tokens,
      prices,
      Pools[i].abi,
      Pools[i].address,
      Pools[i].rewardTokenFunction,
