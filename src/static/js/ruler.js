...
function printRulerContractLinks(
  ...
  for (let i = 0; i < rewardTokenTickers.length; i++) {
    claimText += `${earnedTokens[i].toFixed(fixedDecimals)} ${rewardTokenTickers[i]} ($${formatMoney(
      earnedTokens[i] * rewardTokenPrices[i]
    )}`
  }
...
function printRulerPool(App, rulerAbi, rulerAddr, tokens, prices, poolInfo, lpAddress, poolPrices) {
  ...
  for (let i = 0; i < rewardTokenTickers.length; i++) {
    printAPR(
      rewardTokenTickers[i],
      rewardPrices[i],
      poolInfo.rewardsPerWeek[i],
...
async function loadXRuler(App, tokens, prices) {
  ...
  const XRULER_ABI = [
    {
      inputs: [{internalType: 'contract IERC20', name: '_ruler', type: 'address'}],
      stateMutability: 'nonpayable',
      type: 'constructor',
    },
    {
    ...
    },
    {
      ...
      inputs: [
        {indexed: false, internalType: 'address', name: 'user', type: 'address'},
        {indexed: false, internalType: 'uint256', name: '_ruler', type: 'uint256'},
      ],
      name: 'Deposit',
      type: 'event',
    },
    ...
    },
    {
      inputs: [
        {internalType: 'uint256', name: '_amount', type: 'uint256'},
        {
          ...
          internalType: 'struct IxRULER.Permit',
          name: 'permit',
          type: 'tuple',
        },
      ],
  ...
  const [rulerAddress, userBalance] = await App.ethcallProvider.all([
    XRULER.ruler(),
    XRULER.balanceOf(App.YOUR_ADDRESS),
  ])
...
async function main() {
  ...
  var tokenAddresses = [].concat.apply(
    [],
    poolInfos.filter(x => x.poolToken).map(x => x.poolToken.tokens.concat(x.rewardTokenAddresses))
  )
  ...
  for (i = 0; i < poolPrices.length; i++) {
    printRulerPool(App, RULER_POOL_ABI, RULER_POOL_ADDRESS, tokens, prices, poolInfos[i], lpAddresses[i], poolPrices[i])
  }
