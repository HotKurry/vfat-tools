...
const UNCX_CONTRACT_ABI = [
  {
    inputs: [
      ...
      {internalType: 'address', name: '_farmGenerator', type: 'address'},
    ],
  ...
  {inputs: [], name: 'emergencyWithdraw', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {
    inputs: [],
    name: 'factory',
    outputs: [{internalType: 'contract IFarmFactory', name: '', type: 'address'}],
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'farmableSupply', type: 'uint256'},
  ...
  {
    inputs: [
      {internalType: 'uint256', name: '_from_block', type: 'uint256'},
...
async function loadMultipleUnicryptPools(App, tokens, prices, pools, currentBlockNumber) {
  let totalStaked = 0,
    totalUserStaked = 0,
    individualAPRs = []
  const infos = await Promise.all(pools.map(p => loadUnicryptPoolInfo(App, tokens, prices, p.abi, p.address)))
  let tokenAddresses = [].concat.apply(
    [],
    infos.filter(x => x.stakeToken).map(x => x.stakeToken.tokens)
  )
  let newPrices = await lookUpTokenPrices(tokenAddresses)
  for (const key in newPrices) {
    if (newPrices[key]?.usd) prices[key] = newPrices[key]
  }
  const poolPrices = infos.map(poolInfo =>
    poolInfo.stakeToken ? getPoolPrices(tokens, prices, poolInfo.stakeToken) : undefined
  )
...
async function printUnicryptPool(
  App,
  info,
  poolPrices,
  stakeTokenTicker,
  ...
  _print(
    `You are staking ${info.userStaked.toFixed(8)} ${stakeTokenTicker} ` +
  ...
  _print_link(
    `Claim ${info.earned.toFixed(8)} ${info.rewardTokenTicker} ($${formatMoney(info.earned * rewardTokenPrice)})`,
    claim
  )
  if (stakeTokenTicker != 'ETH') {
