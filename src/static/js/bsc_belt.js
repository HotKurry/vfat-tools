...
async function main() {
  ...
  const rewardsPerWeek = (((await BELT_CHEF.BELTPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()
  prices['0x86aFa7ff694Ab8C985b79733745662760e454169'] = {usd: 1}

  await loadBeltChefContract(
    App,
    tokens,
    prices,
    BELT_CHEF,
...
async function loadBeltChefContract(
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBeltPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
...
function printBeltContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
