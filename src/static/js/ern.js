...
async function main() {
  ...
  const ERN_CHEF_ADDR = '0x34a77Aa9AE42ff9a9B2078E450651D112D5BE908'
  const ERN_CHEF_ABI = [
    {
      inputs: [
        {internalType: 'contract SushiToken', name: '_erc20', type: 'address'},
  ...
  const ERN_CHEF = new ethers.Contract(ERN_CHEF_ADDR, ERN_CHEF_ABI, App.provider)
  const rewardsPerWeek = (((await ERN_CHEF.rewardPerBlock()) / 1e18) * 604800) / 13.5

  await loadChefContract(App, ERN_CHEF, ERN_CHEF_ADDR, ERN_CHEF_ABI, 'ERN', 'erc20', null, rewardsPerWeek, 'pending')
