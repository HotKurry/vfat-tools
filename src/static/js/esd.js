...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.ESD.DAO.address, Contracts.ESD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.ESD.Døllar.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.ESD.Uniswap_ESD_USDC.address,
    Contracts.ESD.LPIncentivizationPool.address,
    tokens,
    prices,
    15
  )
  ...
  const LP = new ethers.Contract(
    Contracts.ESD.LPIncentivizationPool.address,
    Contracts.ESD.LPIncentivizationPool.abi,
    App.provider
  )
  await loadEmptySetLP(App, LP, Contracts.ESD.Uniswap_ESD_USDC.address, 'ESD-USDC LP', 5, epoch, 'ESD', uniPrices)
