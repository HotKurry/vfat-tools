...
const MIMO_STAKING_ABI = [
  {
    inputs: [
      {internalType: 'contract IGovernanceAddressProvider', name: '_addresses', type: 'address'},
...
async function main() {
  ...
  let p3 = await loadMimoContract2(
    App,
    tokens,
    prices,
    MIMO_STAKING_ADDR3,
  ...
  let p4 = await loadMimoContract2(
    App,
    tokens,
    prices,
    MIMO_STAKING_ADDR4,
  ...
  let p5 = await loadMimoContract2(
    App,
    tokens,
    prices,
    MIMO_STAKING_ADDR5,
  ...
  if (
    p1.totalUserStaked > 0 ||
    p2.totalUserStaked > 0 ||
    p3.totalUserStaked > 0 ||
    p4.totalUserStaked > 0 ||
    p5.totalUserStaked > 0
  ) {
    _print_bold(
      `\nYou are staking a total of $${formatMoney(p1.totalUserStaked + p2.totalUserStaked)} at an average APR of ${(
        p1.averageApr +
        p2.averageApr * 100
      ).toFixed(2)}%`
    )
    _print(
      `Estimated earnings:` +
        ` Day $${formatMoney(
          (p1.totalUserStaked * p1.averageApr) / 365 +
        ...
        ` Week $${formatMoney(
          (p1.totalUserStaked * p1.averageApr) / 52 +
        ...
        ` Year $${formatMoney(
          p1.totalUserStaked * p1.averageApr +
...
async function loadMimoContract(
  App,
  tokens,
  prices,
  contractAddress,
...
function printMimoContractLinks(
  App,
  abi,
  contractAddress,
  stakingTokenAddr,
...
const mimoContract_claim = async function(abi, contractAddress, App, pendingRewards, claimFunction) {
  const signer = App.provider.getSigner()
  ...
  if (earnedTokenAmount > 0) {
    showLoading()
    if (claimFunction) {
      claimFunction(App.YOUR_ADDRESS, {gasLimit: 500000}).then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
    } else {
...
async function loadMimoContract2(
  App,
  tokens,
  prices,
  contractAddress,
  ...
  const apr = printMimoPool2(prices, userStaked, poolPrices, rewardsPerWeek, rewardTokenTicker, rewardTokenAddr)
  return apr
}

function printMimoPool2(prices, userStaked, poolPrices, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress) {
  const poolRewardsPerWeek = rewardsPerWeek
  const rewardPrice = getParameterCaseInsensitive(prices, rewardTokenAddress)?.usd
  const staked_tvl = poolPrices.staked_tvl
  poolPrices.print_price()
