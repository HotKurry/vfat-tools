...
const OHM_POOL_ABI = [
  {
    inputs: [
      ...
      {internalType: 'address', name: '_OHMToken', type: 'address'},
...
async function main() {
  ...
  const [ohmPerBlock, lpToken, [userBalance, userEarned]] = await App.ethcallProvider.all([
    OHM_POOL.rewardPerBlock(),
    OHM_POOL.LPToken(),
    OHM_POOL.userDetails(App.YOUR_ADDRESS),
    OHM_POOL.totalStaked(),
  ])
  ...
  _print_link(
    `Claim ${pendingRewardTokens.toFixed(4)} ${rewardTokenTicker} ($${formatMoney(pendingRewardTokens * rewardPrice)})`,
    claim
  )
  hideLoading()
}
