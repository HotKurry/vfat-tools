...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.SPAD.DAO.address, Contracts.SPAD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.SPAD.Dollar.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.SPAD.UniswapLP.address,
    Contracts.SPAD.LPIncentivizationPool.address,
    tokens,
    prices,
  ...
  const LP = new ethers.Contract(
    Contracts.SPAD.LPIncentivizationPool.address,
    Contracts.SPAD.LPIncentivizationPool.abi,
    App.provider
  )
