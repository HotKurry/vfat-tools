...
function printPiedaoContractLinks(
  App,
  piedaoAbi,
  piedaoAddr,
  poolIndex,
...
async function main() {
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount / 1).keys()].map(async x => await getPiedaoPoolInfo(App, PIEDAO_POOL, x))
  )
