...
async function main() {
  ...
  let p0 = await loadPopsicleChefContract(
    App,
    ICE_CHEF0,
    ICE_CHEF_ADDR0,
    ICE_CHEF_ABI,
  ...
  let p1 = await loadPopsicleChefContract(
    App,
    ICE_CHEF1,
    ICE_CHEF_ADDR1,
    ICE_CHEF_ABI,
...
async function loadPopsicleChefContract(
  App,
  chef,
  chefAddress,
  chefAbi,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getPopsiclePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
