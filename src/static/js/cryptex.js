...
const TCAP_STAKING_ABI = [
  {
    inputs: [
      ...
      {internalType: 'uint256', name: '_vestingRatio', type: 'uint256'},
    ],
  ...
  {inputs: [], name: 'claimVest', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {
    inputs: [{internalType: 'address', name: '_account', type: 'address'}],
    name: 'earned',
    outputs: [{internalType: 'uint256', name: '', type: 'uint256'}],
...
async function main() {
  const Pool0 = ['0xc8BB1cd417D20116387a5e0603e195cA4f3Cf59A'].map(a => {
    return {
      address: a,
  ...
  let p1 = await loadCryptexSynthetixPool1(
    App,
    tokens,
    prices,
    Pool1.abi,
    Pool1.address,
    Pool1.rewardTokenFunction,
    Pool1.stakeTokenAddress,
    Pool1.vaultAddress
  )
  let p2 = await loadCryptexSynthetixPool1(
    App,
    tokens,
    prices,
    Pool2.abi,
    Pool2.address,
    Pool2.rewardTokenFunction,
    Pool2.stakeTokenAddress,
    Pool2.vaultAddress
  )
  ...
  if (p0.totalUserStaked > 0 || p1.totalUserStaked > 0 || p2.totalUserStaked > 0) {
    _print(
      `You are staking a total of $${formatMoney(
        p0.totalUserStaked + p1.totalUserStaked + p2.totalUserStaked
      )} at an APR of ${(p0.totalAPR + p1.totalAPR + p2.totalAPR * 100).toFixed(2)}%\n`
    )
  }
...
async function loadCryptexSynthetixPool1(
  App,
  tokens,
  prices,
  abi,
  ...
  const info = await loadCryptexPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadCryptexSynthetixPool2(
  App,
  tokens,
  prices,
  abi,
...
async function loadCryptexPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
