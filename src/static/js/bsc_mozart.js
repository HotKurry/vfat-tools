...
const PIANO_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract PianoToken', name: '_piano', type: 'address'},
      {internalType: 'contract FoamBar', name: '_foam', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'address', name: '_treasuryaddr', type: 'address'},
      {internalType: 'uint256', name: '_pianoPerBlock', type: 'uint256'},
  ...
  },
  {
    inputs: [],
    name: 'foam',
    outputs: [{internalType: 'contract FoamBar', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    inputs: [],
    name: 'piano',
    outputs: [{internalType: 'contract PianoToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accPianoPerShare', type: 'uint256'},
...
async function main() {
  ...
  const rewardsPerWeek = (((await PIANO_CHEF.pianoPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscHawkChefContract(
    App,
    tokens,
    prices,
    PIANO_CHEF,
...
async function loadBscHawkChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async x => await getBscPoolInfo(App, chefContract, chefAddress, x))
  )
...
function printHawkChefContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
...
const chefHawkContract_claim = async function(chefAbi, chefAddress, poolIndex, App, pendingRewards, claimFunction) {
  const signer = App.provider.getSigner()
