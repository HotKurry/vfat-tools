...
const PMP_CHEF_ABI = [
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accPMPPerShare', type: 'uint256'},
...
const STRAT_ABI = [
  {
    inputs: [
      {internalType: 'address', name: '_pumpyFarmAddress', type: 'address'},
      {internalType: 'address', name: '_PMPAddress', type: 'address'},
...
async function main() {
  ...
  const rewardsPerWeek = (((await PMP_CHEF.PMPPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscPumpyContract(
    App,
    tokens,
    prices,
    PMP_CHEF,
...
async function loadBscPumpyContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async x => await getBscPumpyPoolInfo(App, chefContract, x, pendingRewardsFunction))
  )
