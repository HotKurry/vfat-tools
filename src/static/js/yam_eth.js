...
async function main() {
  ...
  const stakingTokenPrice = (prices['yam-2'].usd * yamAmount + prices['ethereum'].usd * wETHAmount) / totalUNIV2Amount
  ...
  _print(
    `There are total   : ${toFixed(
      totalStakedYAmount,
      8
    )} ${stakingTokenTicker} staked in ${rewardTokenTicker}'s ${stakingTokenTicker} staking pool.`
  )
