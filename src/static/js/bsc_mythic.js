...
const MAGIC_ABI = [
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accMagicPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_magicPerBlock', type: 'uint256'}],
    name: 'setMagicPerBlock',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{internalType: 'contract IERC20', name: '_magic', type: 'address'}],
    name: 'setMagicToken',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
const MYTHIC_ABI = [
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'lastRewardBlock', type: 'uint256'},
      {internalType: 'uint256', name: 'accMythicPerShare', type: 'uint256'},
    ],
    stateMutability: 'view',
    type: 'function',
  },
  ...
  {
    inputs: [
      {internalType: 'contract IERC20', name: '_mythic', type: 'address'},
      {internalType: 'contract IERC20', name: '_lpToken', type: 'address'},
    ],
    name: 'setMythicTokens',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [{internalType: 'uint256', name: '_mythicPerBlock', type: 'uint256'}],
    name: 'setmythicPerBlock',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
...
async function main() {
  ...
  const rewardsMagicPerWeek = (((await MAGIC_CONTRACT.magicPerBlock()) / 1e18) * 604800) / 3

  const rewardsMythicPerWeek0 = (((await MYTHIC_CONTRACT0.mythicPerBlock()) / 1e18) * 604800) / 3

  const rewardsMythicPerWeek1 = (((await MYTHIC_CONTRACT1.mythicPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadMythicContract(
    App,
    tokens,
    prices,
    MYTHIC_CONTRACT1,
  ...
  await loadMythicContract(
    ...
    'mythic',
    null,
    rewardsMythicPerWeek0,
    'pendingRewards'
  )

  await loadMagicContract(
    App,
    tokens,
    prices,
    MAGIC_CONTRACT,
...
async function loadMagicContract(
  App,
  tokens,
  prices,
  contract,
  ...
  const poolMagicInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBscMagicPoolInfo(App, chefContract, contractAddress, x, pendingRewardsFunction)
    )
  )

  var tokenAddresses = [].concat.apply(
    [],
    poolMagicInfos.filter(x => x.poolToken).map(x => x.poolToken.tokens)
  )
  ...
  const poolPrices = poolMagicInfos.map(poolInfo =>
...
async function loadMythicContract(
  App,
  tokens,
  prices,
  contract,
