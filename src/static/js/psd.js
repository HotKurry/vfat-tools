...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.PSD.DAO.address, Contracts.PSD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.PSD.PSD.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.PSD.Uniswap_USDC_PSD.address,
    Contracts.PSD.LPIncentivizationPool.address,
    tokens,
    prices,
    15
  )
  ...
  const LP = new ethers.Contract(
    Contracts.PSD.LPIncentivizationPool.address,
    Contracts.PSD.LPIncentivizationPool.abi,
    App.provider
  )
  await loadEmptySetLP(App, LP, Contracts.PSD.Uniswap_USDC_PSD.address, 'PSD-USDC LP', 12, epoch, 'PSD', uniPrices)
