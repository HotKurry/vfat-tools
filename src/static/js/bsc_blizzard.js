...
const BLZD_CHEF_ABI = [
  {
    inputs: [
      ...
      {internalType: 'uint256', name: '_noFeeBlock', type: 'uint256'},
    ],
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accxBLZDPerShare', type: 'uint256'},
...
const STRAT_ABI = [
  {
    inputs: [
      {internalType: 'address', name: '_YetiMasterAddress', type: 'address'},
      {internalType: 'address', name: '_xBLZDAddress', type: 'address'},
      {internalType: 'bool', name: '_isCAKEStaking', type: 'bool'},
      {internalType: 'bool', name: '_isxBLZDComp', type: 'bool'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_wantAmt', type: 'uint256'}],
    name: 'deposit',
    outputs: [{internalType: 'uint256', name: '', type: 'uint256'}],
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const rewardsPerWeek = (((await BLZD_CHEF.xBLZDPerBlock()) / 1e18) * 604800) / 3
...
async function loadBscChefContract(
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBlizzardPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
