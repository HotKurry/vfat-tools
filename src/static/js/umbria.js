...
async function main() {
  ...
  const UMBR_CHEF_ADDR = '0xdF9401225cC62d474C559E9c4558Fb193137bCEB'
  const UMBR_CHEF_ABI = [
    {
      inputs: [
        {internalType: 'contract SushiToken', name: '_umbria', type: 'address'},
        {internalType: 'address', name: '_devaddr', type: 'address'},
        {internalType: 'uint256', name: '_umbriaPerBlock', type: 'uint256'},
  ...
  const UMBR_CHEF = new ethers.Contract(UMBR_CHEF_ADDR, UMBR_CHEF_ABI, App.provider)
  const rewardsPerWeek = (((await UMBR_CHEF.umbriaPerBlock()) / 1e18) * 604800) / 13.5
