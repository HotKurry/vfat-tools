...
async function loadCoverprotocolContracts(App, tokens, prices, pools, coverContract) {
  ...
  const infos = await Promise.all(
    pools.map(p => loadCoverprotocolPoolInfo(App, tokens, p.address, p.stakedToken, p.rewards, coverContract))
  )
  let tokenAddresses = []
...
async function loadCoverprotocolPoolInfo(App, tokens, stakingAddress, stakeTokenAddress, rewards, coverContract) {
...
async function printCoverSynthetixPool(App, tokens, prices, info, chain = 'eth') {
  ...
  for (const rewardToken of info.rewardTokens) {
    ...
    _print(
      `${rewardToken.rewardTokenTicker} Per Week: ${rewardToken.weeklyRewards.toFixed(2)} ($${formatMoney(
        rewardToken.usdPerWeek
      )})`
    )
  ...
  for (const rewardToken of info.rewardTokens) {
    _print_link(
      `Claim ${rewardToken.earned.toFixed(6)} ${rewardToken.rewardTokenTicker} ($${formatMoney(
        rewardToken.earned * rewardToken.rewardTokenPrice
      )})`,
      claim
    )
  }
