...
const CRL_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract CRLToken', name: '_crl', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'address', name: '_feeAddress', type: 'address'},
      {internalType: 'uint256', name: '_crlPerBlock', type: 'uint256'},
  ...
  },
  {
    ...
    inputs: [
      {indexed: true, internalType: 'address', name: 'user', type: 'address'},
      {indexed: false, internalType: 'uint256', name: 'crlPerBlock', type: 'uint256'},
    ],
  ...
  },
  {
    inputs: [],
    name: 'crl',
    outputs: [{internalType: 'contract CRLToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accCrlPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_crlPerBlock', type: 'uint256'}],
    name: 'updateEmissionRate',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const CRL_CHEF_ADDR = '0x713e34640ef300a0B178a9688458BbA8b1FA35A7'
  const rewardTokenTicker = 'CRL'
  const CRL_CHEF = new ethers.Contract(CRL_CHEF_ADDR, CRL_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await CRL_CHEF.crlPerBlock()) / 1e18) * 604800) / 3
