...
async function main() {
  ...
  for (const key in data.poolInfos) {
    try {
      ...
      if (userStaked > 0) {
        ...
        _print(
          `Est earning: ` +
            `Daily ${parseFloat(dailyReward / prices[rewardToken]).toFixed(2)} ${rewardToken}  ($${formatMoney(
              dailyReward
            )})  ` +
            `Monthly: ${parseFloat(monthlyReward / prices[rewardToken]).toFixed(2)} ${rewardToken} ($${formatMoney(
              monthlyReward
            )})  ` +
            `Yearly: ${parseFloat(yearlyReward / prices[rewardToken]).toFixed(2)} ${rewardToken} ($${formatMoney(
              yearlyReward
            )}) `
        )
      }
      _print_link(
        `Approve ${parseFloat(stakedTokenBalance / 10 ** decimal).toFixed(8)}  ${Contracts.BDOLLAR[key].symbol}`,
        approveFunc
      )
      _print_link(
        `Stake ${parseFloat(stakedTokenBalance / 10 ** decimal).toFixed(8)}  ${Contracts.BDOLLAR[key].symbol}`,
        stakeFunc
      )
  ...
  const boardRoomContract = new ethers.Contract(
    Contracts.BDOLLAR[boardRoomKey].address,
    Contracts.BDOLLAR[boardRoomKey].abi,
  ...
  const sBDOContract = new ethers.Contract(
    sBDOAddress,
    Contracts.BDOLLAR.Parameters.BEPToken.abi,
    App.provider.getSigner()
  )
  ...
  _print(
    `There are ${b.sbdoLocked} sBDO in boardroom, ${parseFloat(
      (b.sbdoLocked * 100) / sBDOTokenInfo.circulatingSupply
    ).toFixed(2)}% of circulating`
  )
  ...
  _print(`APR ${apr} %`)
  _print(
    `You are staking ${inBoardRoom.toFixed(2)} sBDO ($${formatMoney(inBoardRoomUsd)}) in boardroom, ${parseFloat(
      (inBoardRoom * 100) / b.sbdoLocked
