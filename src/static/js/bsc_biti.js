...
const BITI_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'address', name: '_biti', type: 'address'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accBitiPerShare', type: 'uint256'},
...
async function main() {
  ...
  const rewardsPerWeek = (((await BITI_CHEF.bitiPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscBitiChefContract(
    App,
    tokens,
    prices,
    BITI_CHEF,
...
async function loadBscBitiChefContract(
  App,
  tokens,
  prices,
  chef,
