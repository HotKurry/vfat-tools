...
const LKM_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract Token', name: '_lkm', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'address', name: '_feeAddress', type: 'address'},
      {internalType: 'uint256', name: '_lkmPerBlock', type: 'uint256'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accLkmPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_lkmPerBlock', type: 'uint256'}],
    name: 'updateEmissionRate',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const rewardsPerWeek = (((await LKM_CHEF.lkmPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscLokumContract(
    App,
    tokens,
    prices,
    LKM_CHEF,
...
async function loadBscLokumContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBscLokumPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
