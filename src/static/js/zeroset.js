...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.ZSD.DAO.address, Contracts.ZSD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.ZSD.Dollar.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.ZSD.UniswapLP.address,
    Contracts.ZSD.LPIncentivizationPool.address,
    tokens,
    prices,
  ...
  const LP = new ethers.Contract(
    Contracts.ZSD.LPIncentivizationPool.address,
    Contracts.ZSD.LPIncentivizationPool.abi,
    App.provider
  )
