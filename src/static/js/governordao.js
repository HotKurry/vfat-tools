...
const GDAO_FARMING_ABI = [
  {
    inputs: [
      {internalType: 'contract IERC20', name: 'rewardsToken', type: 'address'},
      {internalType: 'contract IERC20', name: 'stakingToken', type: 'address'},
      {internalType: 'uint8', name: 'stakingTokenDecimals', type: 'uint8'},
      {internalType: 'uint256', name: 'periodStartTime_', type: 'uint256'},
      {internalType: 'uint256', name: 'periodEndTime_', type: 'uint256'},
    ],
...
async function main() {
  ...
  let p = await loadGDAOSynthetixPool(
    App,
    tokens,
    prices,
    GDAO_FARMING_ABI,
    GDAO_CONTRACT_ADDRESS,
    rewardTokenFunction,
    stakeTokenFunction
  )
...
async function loadGDAOSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadGDAOSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadGDAOSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
