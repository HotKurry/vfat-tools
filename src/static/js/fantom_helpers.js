const FantomTokens = [{id: 'tether', symbol: 'USDT', contract: '0x049d68029688eAbF473097a2fC38ef61633A3C7A'}]

async function getFantomPrices() {
  const idPrices = await lookUpPrices(FantomTokens.map(x => x.id))
  const prices = {}
  for (const bt of FantomTokens) if (idPrices[bt.id]) prices[bt.contract] = idPrices[bt.id]
  return prices
}
...
async function loadFantomSynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
...
async function loadFantomSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomSynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadFantomBasisFork(data) {
  ...
  let p1 = await loadFantomSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadFantomSynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadFantomSynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
...
async function loadFantomChefContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getFantomPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
  ...
  if (deathPoolIndices) {
    ...
    deathPoolIndices
      .map(i => poolInfos[i])
      .map(poolInfo => (poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, 'Fantom') : undefined))
  }
...
async function loadMultipleFantomSynthetixPoolsSequential(App, tokens, prices, pools) {
  ...
  for (const p of pools) {
    let res = await loadFantomSynthetixPool(
      App,
      tokens,
      prices,
      p.abi,
