...
async function loadPool(
  ...
  if (poolInfo.stakedToken != null) {
    ...
    _print(
      `RTC APR: Day ${(dailyAPR * 10000).toFixed(2)}% Week ${(weeklyAPR * 10000).toFixed(2)}% Year ${(
        yearlyAPR * 10000
      ).toFixed(2)}%`
    )
    ...
    _print(
      `You are staking ${userStaked.toFixed(8)} ${pp.stakingTokenTicker} ($${formatMoney(
        userStakedUsd
