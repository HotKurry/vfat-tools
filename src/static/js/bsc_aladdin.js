...
const WISH_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract WishToken', name: '_wish', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'address', name: '_feeAddress', type: 'address'},
      {internalType: 'uint256', name: '_wishPerBlock', type: 'uint256'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accWishPerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_wishPerBlock', type: 'uint256'}],
  ...
  },
  {
    inputs: [],
    name: 'wish',
    outputs: [{internalType: 'contract WishToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
...
async function main() {
  ...
  const WISH_CHEF_ADDR = '0x141CCd27BBcA7524dC6b7eEa1eb41E097D7E832D'
  const rewardTokenTicker = 'WISH'
  const WISH_CHEF = new ethers.Contract(WISH_CHEF_ADDR, WISH_CHEF_ABI, App.provider)

  const startBlock = await WISH_CHEF.startBlock()
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = (((await WISH_CHEF.wishPerBlock()) / 1e18) * 604800) / 3
  }
