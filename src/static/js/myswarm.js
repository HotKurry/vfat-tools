...
const SWM_STAKING_ABI = [
  ...
  {
    inputs: [],
    name: 'avgRewardsPerSecondTotal',
    outputs: [{internalType: 'uint256', name: 'avgPerSecond', type: 'uint256'}],
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accUndistributedReward', type: 'uint256'},
    ],
    stateMutability: 'view',
    type: 'function',
  },
...
async function main() {
  ...
  const rewardsPerWeek = ((await SWM_CHEF.avgRewardsPerSecondTotal()) / 1e18) * 604800

  await loadChefSwarmContract(
    App,
    SWM_CHEF,
    SWM_CHEF_ADDR,
    SWM_STAKING_ABI,
...
async function loadChefSwarmContract(
  App,
  chef,
  chefAddress,
  chefAbi,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getSwarmPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
