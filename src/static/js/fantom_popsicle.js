...
async function main() {
  ...
  await loadFantomPopsicleContract(
    App,
    tokens,
    prices,
    FANTOM_ICE_CHEF0,
    FANTOM_ICE_CHEF_ADDR0,
    FANTOM_ICE_CHEF_ABI,
    rewardTokenTicker,
    'ice',
    null,
...
async function loadFantomPopsicleContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getFantomPopsiclePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
