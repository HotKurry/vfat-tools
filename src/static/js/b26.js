...
const B26_CONTRACT_ABI = [
  ...
  {inputs: [], name: 'claimDivs', outputs: [], stateMutability: 'nonpayable', type: 'function'},
  {
    inputs: [],
    name: 'cliffTime',
...
async function loadMultipleSynthetixB26_Pools(App, tokens, prices, pools) {
  ...
  const infos = await Promise.all(
    pools.map(p => loadSynthetixB26_PoolInfo(App, tokens, prices, p.abi, p.address, p.tokenFunction))
  )
...
async function loadSynthetixB26_PoolInfo(App, tokens, prices, stakingAbi, stakingAddress, tokenFunction) {
  ...
  const calls = [
    ...
    STAKING_MULTI.depositedTokens(App.YOUR_ADDRESS),
    STAKING_MULTI.totalEarnedTokens(App.YOUR_ADDRESS),
  ]
