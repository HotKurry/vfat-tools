...
const THUNDER_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract ThunderToken', name: '_thunder', type: 'address'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accThunderPerShare', type: 'uint256'},
  ...
  },
  {
    inputs: [],
    name: 'thunder',
    outputs: [{internalType: 'contract ThunderToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
...
async function main() {
  ...
  const THUNDER_CHEF_ADDR = '0xa7bfbefbe923dcf6eeff355c22520395670b684e'
  const rewardTokenTicker = 'TNDR'
  const THUNDER_CHEF = new ethers.Contract(THUNDER_CHEF_ADDR, THUNDER_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await THUNDER_CHEF.thunderPerBlock()) / 1e18) * 604800) / 3
