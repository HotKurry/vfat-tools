...
const calculateChange = (price, totalCoupons, totalRedeemable) => {
  if (price > 1.06 && totalCoupons > totalRedeemable) {
    return 0.06
  } else if (1.06 >= price && price > 1.03 && totalCoupons > totalRedeemable) {
...
async function main() {
  ...
  const DAO = new ethers.Contract(Contracts.BSD.DAO.address, Contracts.BSD.DAO.abi, App.provider)
  const DOLLAR = new ethers.Contract(Contracts.BSD.BSD.address, ERC20_ABI, App.provider)
  var prices = {}
  var tokens = {}
  ...
  const [epoch, uniPrices, totalBonded] = await loadDAO(
    ...
    DAO,
    DOLLAR,
    Contracts.BSD.Uniswap_BSD_USDC.address,
    Contracts.BSD.LPIncentivizationPool.address,
    tokens,
    prices,
    72
  )
  ...
  const LP = new ethers.Contract(
    Contracts.BSD.LPIncentivizationPool.address,
    Contracts.BSD.LPIncentivizationPool.abi,
    App.provider
  )
  await loadEmptySetLP(App, LP, Contracts.BSD.Uniswap_BSD_USDC.address, 'BSD-USDC LP', 24, epoch, 'BSD', uniPrices)
  ...
  if (epoch < 301) {
    ...
    _print(
      `LP  APR: Day ${lpReturn.toFixed(2)}% Week ${(lpReturn * 7).toFixed(2)}% Year ${(lpReturn * 365).toFixed(2)}%`
    )
  } else {
    ...
    if (array.length > 0) {
      ...
      if (twap > 1.05) {
        ...
        _print(
          `LP  APR: Day ${(lpReturn * 3).toFixed(2)}% Week ${(lpReturn * 3 * 7).toFixed(2)}% Year ${(
            lpReturn *
            3 *
            365
          ).toFixed(2)}%`
