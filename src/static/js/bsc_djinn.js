...
const DJINN_CHEF_ABI = [
  ...
  },
  {
    inputs: [
      ...
      {internalType: 'uint256', name: '_feeFactor', type: 'uint256'},
    ],
    name: 'addPool',
    outputs: [],
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accumulatedRewardPerToken', type: 'uint256'},
      {internalType: 'bool', name: 'isStarted', type: 'bool'},
      {internalType: 'uint256', name: 'feeFactor', type: 'uint256'},
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'payoutOffset', type: 'uint256'},
    ],
    stateMutability: 'view',
    type: 'function',
  },
...
async function main() {
  ...
  const rewardsPerWeek = (((await DJINN_CHEF.COIN_PER_BLOCK()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscDjinnContract(
    App,
    tokens,
    prices,
    DJINN_CHEF,
...
async function loadBscDjinnContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(
      async x => await getBscDjinnPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
    )
  )
